...
import co.com.bancolombia.declaraciones.services.service.IFormularios2ExcelProcessorService;
...
@Service
public class Formularios2ExcelProcessorServiceImpl implements IFormularios2ExcelProcessorService {
    private List<MassiveLoadErrorDTO> errors;
    private List<MLExportacionesDto> formOneRegisters;
    ...
    public void processData(XSSFWorkbook workbook) {
        ...
        while (rowIterator.hasNext()) {
            ...
            Cell refundCurrencyCodeCell = row.getCell(11);
            Cell refundCurrencyValueCell = row.getCell(12);
            Cell typeChangeToUsdCell = row.getCell(13);
            Cell numeralCell = row.getCell(14);
            Cell customsOfficerNumberDocumentCell = row.getCell(15);
            Cell refundUsdValueCell = row.getCell(16);
            Cell numeralExchangeDeductionsCell = row.getCell(17);
            Cell customsOfficerInformationCell = row.getCell(18);
            Cell observationsCell = row.getCell(19);
            if (rowCounter >= lineToStart) {
                ...
                try {
                    ...
                    MLExportacionesDto mlExportacionesDto = new MLExportacionesDto();
                    mlExportacionesDto.setDocumentType(documentTypeStr);


                    mlExportacionesDto.setDocumentNumber((long) Double.parseDouble(documentNumberStr));
                    ...
                    if (documentTypeStr.equalsIgnoreCase("NI")) {
                        if (dvStr.equalsIgnoreCase(StringUtils.EMPTY)) {
                            ...
                            mlExportacionesDto.setValidationDigit((int) Double.parseDouble(dvStr));
                        }
                    ...
                    mlExportacionesDto.setSocialReasonName(this.getValueFromCell(socialReasonNameCell, false));
                    mlExportacionesDto.setCompensationAccountCode(this.getValueFromCell(compensationAccountCodeCell, false));
                    mlExportacionesDto.setFinanceEntityAccountNumber(this.getValueFromCell(financeEntityAccountNumberCell, false));
                    String typeOperationStr = this.getValueFromCell(typeOperationCell, false);
                    mlExportacionesDto.setTypeOperation(typeOperationStr);
                    ...
                    if (formDateStr.equalsIgnoreCase(StringUtils.EMPTY)){
                        ...
                        mlExportacionesDto.setFormDate(this.massiveLoadExcelProcessorHelper.getDateFromString(formDateStr));

                    }

                    mlExportacionesDto.setConsecutive(this.getValueFromCell(consecutiveCell, false));
                    ...
                    if (typeOperationStr.equalsIgnoreCase(StringUtils.EMPTY)) {
                        ...
                        if (typeOperationStr.substring(0, 2).equalsIgnoreCase("3.") || typeOperationStr.substring(0, 2).equalsIgnoreCase("4.")) {
                            if (documentOldDateStr.equalsIgnoreCase(StringUtils.EMPTY)) {
                                ...
                                mlExportacionesDto.setDocumentOldDate(this.massiveLoadExcelProcessorHelper.getDateFromString(documentOldDateStr));
                            }
                           ...
                           mlExportacionesDto.setConsecutiveOld(consecutiveOldStr);
                        } else {
                            if (!documentOldDateStr.equalsIgnoreCase(StringUtils.EMPTY)) {
                    ...
                    mlExportacionesDto.setRefundCurrencyCode(this.getValueFromCell(refundCurrencyCodeCell, false));

                    String refundCurrencyValueStr = this.getValueFromCell(refundCurrencyValueCell, true);
                    if (refundCurrencyValueStr.equalsIgnoreCase(StringUtils.EMPTY)) {
                        this.addErrorToList(this.massiveLoadExcelProcessorHelper.getCellDescription(refundCurrencyValueCell.getColumnIndex(), rowCounter), "Debes diligenciar el valor moneda de reintegro");
                    } else {
                        mlExportacionesDto.setRefundCurrencyValue(BigDecimal.valueOf(Double.parseDouble(refundCurrencyValueStr)));
                    }
                    ...
                    if (typeChangeUsdStr.equalsIgnoreCase(StringUtils.EMPTY)) {
                        ...
                        mlExportacionesDto.setTypeChangeToUsd(BigDecimal.valueOf(Double.parseDouble(typeChangeUsdStr)));
                    }
                    ...
                    if (numeralStr.equalsIgnoreCase(StringUtils.EMPTY)) {
                        ...
                        mlExportacionesDto.setNumeral((int) Double.parseDouble(numeralStr));
                    }
                    ...
                    if (!customsOfficerNumberDocumentStr.equalsIgnoreCase(StringUtils.EMPTY)) {
                        mlExportacionesDto.setCustomsOfficerNumberDocument(customsOfficerNumberDocumentStr);

                        String refundUsdValueValueStr = this.getValueFromCell(refundUsdValueCell, true);
                        if (refundUsdValueValueStr.equalsIgnoreCase(StringUtils.EMPTY)) {
                            addErrorToList(this.massiveLoadExcelProcessorHelper.getCellDescription(refundUsdValueCell.getColumnIndex(), rowCounter),
                                    "Debes diliegnciar el campo 'Valor reintegro USD' debido a que tienes diligenciado el documento aduanero");
                        } else {
                            mlExportacionesDto.setRefundUsdValue(BigDecimal.valueOf(Double.parseDouble(refundUsdValueValueStr)));
                        }
                    }

                    String numeralExchangeDeductionsStr = this.getValueFromCell(numeralExchangeDeductionsCell, true);
                    if (!numeralExchangeDeductionsStr.equals(StringUtils.EMPTY)) {
                        mlExportacionesDto.setNumeralExchangeDeductions(BigDecimal.valueOf(Double.parseDouble(numeralExchangeDeductionsStr)));
                    }
                    mlExportacionesDto.setCustomsOfficerInformation(this.getValueFromCell(customsOfficerInformationCell, false));
                    mlExportacionesDto.setObservations(this.getValueFromCell(observationsCell, false));


                    validateLines(mlExportacionesDto, rowCounter);
                    this.formOneRegisters.add(mlExportacionesDto);
    ...
    @Override
    public List<MLExportacionesDto> getRegisters() {
    ...
    private void validateLines(MLExportacionesDto mlExportacionesDto, int rowCounter) {
        if (mlExportacionesDto.getDocumentType().equalsIgnoreCase(StringUtils.EMPTY)) {
        ...
        if (mlExportacionesDto.getSocialReasonName().equals(StringUtils.EMPTY)) {
        ...
        if (mlExportacionesDto.getCompensationAccountCode().equals(StringUtils.EMPTY)) {
        ...
        if (mlExportacionesDto.getFinanceEntityAccountNumber().equals(StringUtils.EMPTY)) {
        ...
        if (mlExportacionesDto.getConsecutive().equals(StringUtils.EMPTY)) {
        ...
        if (mlExportacionesDto.getRefundCurrencyCode().equals(StringUtils.EMPTY)) {
        ...
        if (mlExportacionesDto.getCustomsOfficerInformation().equals(StringUtils.EMPTY)) {
